...
interface SubmitToInzModalProps {
    applicationId: string;
    onClose: () => void;
    onSubmit: (inzReference: string) => Promise<void>;
    loading: boolean;
}

export function SubmitToInzModal({
                                     applicationId,
                                     onClose,
                                     onSubmit,
                                     loading,
                                 }: SubmitToInzModalProps) {
    const [inzReference, setInzReference] = useState('');

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!inzReference.trim()) return;
        await onSubmit(inzReference.trim());
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="mb-1.5 block text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                            INZ Reference Number <span className="text-error-500">*</span>
                        </label>
                        <input
                            type="text"
                            value={inzReference}
                            onChange={(e) => setInzReference(e.target.value)}
                        ...
                        <p className="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">
                            Enter the reference number provided by the immigration authority
                        </p>
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button
                            type="submit"
                            disabled={!inzReference.trim() || loading}
